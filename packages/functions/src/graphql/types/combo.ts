...
export const ComboType = builder.objectRef<SQL.Row["combo"]>("Combo").implement({
  fields: (t) => {
    return ({
      id: t.exposeInt("id"),
...
const ProductInputRef = builder.inputType('ProductInput', {
  fields: (t) => ({
    product_id: t.int({ required: true }),
    price: t.float({ required: true }),
    dish_type: t.string({ required: true }),
  }),
});
...
builder.queryFields((t)=> ({
  comboById: t.field({
    ...
    resolve: (_, args) => Combo.getCombo(args.id),
  }),
  combosList: t.field({
    type: [ComboType],
